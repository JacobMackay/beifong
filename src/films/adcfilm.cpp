 ...
 public:
     ...
     ADCFilm(const Properties &props) : Base(props) {
         ...
         if (file_format == "openexr" || file_format == "exr") {
             ...
             Throw("The \"file_format\" parameter must be "
                  "equal to \"openexr\", found %s instead.", file_format);
         }
         ...
         if (pixel_format == "luminance" || is_monochromatic_v<Spectrum>) {
             ...
             Throw("The \"pixel_format\" parameter must be equal to "
                "\"luminance\". Found %s.", pixel_format);
         }
     ...
     bool develop(const ScalarPoint2i  &source_offset,
         ...
         const uint8_t *source_data = source->uint8_data()
             + (source_offset.x() + source_offset.y() * source->width())
             * source_bpp;
         uint8_t *target_data = target->uint8_data()
             + (target_offset.x() + target_offset.y() * target->width())
             * target_bpp;

         if (size.x() == m_crop_size.x()
     ...
     ref<Bitmap> bitmap(bool raw = false) override {
         ...
         ref<Bitmap> source = new Bitmap(Bitmap::PixelFormat::MultiChannel,
                           struct_type_v<ScalarFloat>, m_storage->size(),
                           m_storage->channel_count(),
                           reinterpret_cast<uint8_t *>(
                               m_storage->data().managed().data()));

         if (raw) {
             return source;
         }

         bool has_aovs = m_channels.size() != 3;
     ...
     void develop() override {
         ...
         std::string extension
            = string::to_lower(filename.extension().string());
...
MTS_IMPLEMENT_CLASS_VARIANT(ADCFilm, Film)
MTS_EXPORT_PLUGIN(ADCFilm, "ADC Film")
