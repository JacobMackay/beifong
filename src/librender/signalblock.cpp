...
MTS_VARIANT
SignalBlock<Float, Spectrum>::SignalBlock(const ScalarVector2i &size, size_t channel_count,
...
MTS_VARIANT SignalBlock<Float, Spectrum>::~SignalBlock() {
...
MTS_VARIANT void SignalBlock<Float, Spectrum>::clear() {
...
MTS_VARIANT void SignalBlock<Float, Spectrum>::set_size(const ScalarVector2i &size) {
    if (size == m_size)
        return;
    m_size = size;
...
MTS_VARIANT void SignalBlock<Float, Spectrum>::put(const SignalBlock *block) {
    ScopedPhase sp(ProfilerPhase::SignalBlockPut);

    if (unlikely(block->channel_count() != channel_count()))
        Throw("SignalBlock::put(): mismatched channel counts!");
...
MTS_VARIANT typename SignalBlock<Float, Spectrum>::Mask
SignalBlock<Float, Spectrum>::put(const Point2f &pos_, const Float *value, Mask active) {
    ScopedPhase sp(ProfilerPhase::SignalBlockPut);
...
MTS_VARIANT std::string SignalBlock<Float, Spectrum>::to_string() const {
    std::ostringstream oss;
    oss << "SignalBlock[" << std::endl
...
MTS_IMPLEMENT_CLASS_VARIANT(SignalBlock, Object)
MTS_INSTANTIATE_CLASS(SignalBlock)
