...
// Perhaps I can do polymorphism and do 2d, 3d and 4d images? Or do I do it as variants?
// For now I will stick with 4
...
// 3d ------------------------------------------------------------------
MTS_VARIANT
ImageBlock<Float, Spectrum>::ImageBlock(const ScalarVector3i &size, size_t channel_count,
                                        const ReconstructionFilter *filter, bool warn_negative,
                                        bool warn_invalid, bool border, bool normalize)
    : m_offset(0), m_size(0), m_channel_count((uint32_t) channel_count), m_filter(filter),
      m_weights_x(nullptr), m_weights_y(nullptr), m_weights_z(nullptr), m_warn_negative(warn_negative),
      m_warn_invalid(warn_invalid), m_normalize(normalize) {
    m_border_size = (uint32_t)((filter != nullptr && border) ? filter->border_size() : 0);

    if (filter) {
        // Temporary buffers used in put()
        int filter_size = (int) std::ceil(3 * filter->radius()) + 1;
        m_weights_x = new Float[3 * filter_size];
        m_weights_y = m_weights_x + filter_size;
        m_weights_z = m_weights_x + filter_size;
    }
...
MTS_VARIANT void ImageBlock<Float, Spectrum>::clear() {
    size_t size = m_channel_count * hprod(m_size + 3 * m_border_size);
    if constexpr (!is_cuda_array_v<Float>)
        memset(m_data.data(), 0, size * sizeof(ScalarFloat));
    else
        m_data = zero<DynamicBuffer<Float>>(size);
...
MTS_VARIANT void ImageBlock<Float, Spectrum>::set_size(const ScalarVector3i &size) {
    if (size == m_size)
        return;
    m_size = size;
    m_data = empty<DynamicBuffer<Float>>(
        m_channel_count * hprod(size + 3 * m_border_size));
}
...
MTS_VARIANT void ImageBlock<Float, Spectrum>::put(const ImageBlock *block) {
    ...
    ScalarVector3i source_size   = block->size() + 3 * block->border_size(),
                   target_size   =        size() + 3 *        border_size();

    ScalarPoint3i  source_offset = block->offset() - block->border_size(),
                   target_offset =        offset() -        border_size();
    ...
    if constexpr (is_cuda_array_v<Float> || is_diff_array_v<Float>) {
        accumulate_3d<Float &, const Float &>(
            block->data(), source_size,
            data(), target_size,
            ScalarVector3i(0), source_offset - target_offset,
            source_size, channel_count()
        );
    } else {
        accumulate_3d(
...
MTS_VARIANT typename ImageBlock<Float, Spectrum>::Mask
ImageBlock<Float, Spectrum>::put(const Point3f &pos_, const Float *value, Mask active) {
    ScopedPhase sp(ProfilerPhase::ImageBlockPut);
    Assert(m_filter != nullptr);
    ...
    ScalarFloat filter_radius = m_filter->radius();
    ScalarVector3i size = m_size + 3 * m_border_size;

    // Convert to pixel coordinates within the image block
    Point3f pos = pos_ - (m_offset - m_border_size + .5f);

    if (filter_radius > 0.5f + math::RayEpsilon<Float>) {
        // Determine the affected range of pixels
        Point3u lo = Point3u(max(ceil2int <Point3i>(pos - filter_radius), 0)),
                hi = Point3u(min(floor2int<Point3i>(pos + filter_radius), size - 1));

        uint32_t n = ceil2int<uint32_t>((m_filter->radius() - 3.f * math::RayEpsilon<ScalarFloat>) * 3.f);
        ...
        for (uint32_t i = 0; i < n; ++i) {
            ...
            if constexpr (!is_cuda_array_v<Float>) {
                m_weights_x[i] = m_filter->eval_discretized(p.x(), active);
                m_weights_y[i] = m_filter->eval_discretized(p.y(), active);
                m_weights_z[i] = m_filter->eval_discretized(p.z(), active);
            } else {
                m_weights_x[i] = m_filter->eval(p.x(), active);
                m_weights_y[i] = m_filter->eval(p.y(), active);
                m_weights_z[i] = m_filter->eval(p.z(), active);
            }
        }
        ...
        Point3u lo = ceil2int<Point3i>(pos - .5f);
        UInt32 offset = m_channel_count * (lo.y() * size.x() + lo.x());
...
// 4d ------------------------------------------------------------------
MTS_VARIANT
ImageBlock<Float, Spectrum>::ImageBlock(const ScalarVector4i &size, size_t channel_count,
                                        const ReconstructionFilter *filter, bool warn_negative,
                                        bool warn_invalid, bool border, bool normalize)
    : m_offset(0), m_size(0), m_channel_count((uint32_t) channel_count), m_filter(filter),
      m_weights_x(nullptr), m_weights_y(nullptr), m_weights_z(nullptr), m_weights_w(nullptr), m_warn_negative(warn_negative),
      m_warn_invalid(warn_invalid), m_normalize(normalize) {
    m_border_size = (uint32_t)((filter != nullptr && border) ? filter->border_size() : 0);

    if (filter) {
        // Temporary buffers used in put()
        int filter_size = (int) std::ceil(4 * filter->radius()) + 1;
        m_weights_x = new Float[4 * filter_size];
        m_weights_y = m_weights_x + filter_size;
        m_weights_z = m_weights_x + filter_size;
        m_weights_w = m_weights_x + filter_size;
    }
...
MTS_VARIANT void ImageBlock<Float, Spectrum>::clear() {
    size_t size = m_channel_count * hprod(m_size + 4 * m_border_size);
    if constexpr (!is_cuda_array_v<Float>)
        memset(m_data.data(), 0, size * sizeof(ScalarFloat));
    else
        m_data = zero<DynamicBuffer<Float>>(size);
...
MTS_VARIANT void ImageBlock<Float, Spectrum>::set_size(const ScalarVector4i &size) {
    if (size == m_size)
        return;
    m_size = size;
    m_data = empty<DynamicBuffer<Float>>(
        m_channel_count * hprod(size + 4 * m_border_size));
}
...
MTS_VARIANT void ImageBlock<Float, Spectrum>::put(const ImageBlock *block) {
    ...
    ScalarVector4i source_size   = block->size() + 4 * block->border_size(),
                   target_size   =        size() + 4 *        border_size();

    ScalarPoint4i  source_offset = block->offset() - block->border_size(),
                   target_offset =        offset() -        border_size();

    if constexpr (is_cuda_array_v<Float> || is_diff_array_v<Float>) {
        accumulate_4d<Float &, const Float &>(
            block->data(), source_size,
            data(), target_size,
            ScalarVector4i(0), source_offset - target_offset,
            source_size, channel_count()
        );
    } else {
        accumulate_4d(
...
MTS_VARIANT typename ImageBlock<Float, Spectrum>::Mask
ImageBlock<Float, Spectrum>::put(const Point4f &pos_, const Float *value, Mask active) {
    ScopedPhase sp(ProfilerPhase::ImageBlockPut);
    Assert(m_filter != nullptr);
    ...
    ScalarFloat filter_radius = m_filter->radius();
    ScalarVector4i size = m_size + 4 * m_border_size;

    // Convert to pixel coordinates within the image block
    Point4f pos = pos_ - (m_offset - m_border_size + .5f);

    if (filter_radius > 0.5f + math::RayEpsilon<Float>) {
        // Determine the affected range of pixels
        Point4u lo = Point4u(max(ceil2int <Point4i>(pos - filter_radius), 0)),
                hi = Point4u(min(floor2int<Point4i>(pos + filter_radius), size - 1));

        uint32_t n = ceil2int<uint32_t>((m_filter->radius() - 4.f * math::RayEpsilon<ScalarFloat>) * 4.f);
        ...
        for (uint32_t i = 0; i < n; ++i) {
            ...
            if constexpr (!is_cuda_array_v<Float>) {
                ...
                m_weights_y[i] = m_filter->eval_discretized(p.y(), active);
                m_weights_z[i] = m_filter->eval_discretized(p.z(), active);
                m_weights_w[i] = m_filter->eval_discretized(p.w(), active);
            } else {
                m_weights_x[i] = m_filter->eval(p.x(), active);
                m_weights_y[i] = m_filter->eval(p.y(), active);
                m_weights_z[i] = m_filter->eval(p.z(), active);
                m_weights_w[i] = m_filter->eval(p.w(), active);
            }
        }
        ...
        Point4u lo = ceil2int<Point4i>(pos - .5f);
        UInt32 offset = m_channel_count * (lo.y() * size.x() + lo.x());
